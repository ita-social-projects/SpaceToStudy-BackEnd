...
const validateQuiz = (resource) => {
  ...
  if (typeof resource.settings !== 'object' || Array.isArray(resource.settings)) {
    throw createError(400, FIELD_IS_NOT_OF_PROPER_TYPE('resource settings', 'object'))
  }
  ...
  for (const property of QUIZ_SETTINGS_ENUM) {
    if (!(property in resource.settings)) {
      throw createError(400, FIELD_IS_NOT_DEFINED(`${property} of settings`))
    }

    if (typeof resource.settings[property] !== 'boolean') {
      throw createError(400, FIELD_IS_NOT_OF_PROPER_TYPE(`${property} of settings`, 'boolean'))
    }
  }

  if (resource.items) {
    if (!Array.isArray(resource.items)) {
      throw createError(400, FIELD_IS_NOT_OF_PROPER_TYPE('resource items', 'array'))
    }
